...
public class Proxies {
  public static <T> T makeSynchronized(Class<T> subjectIntf,T realSubject) {
    SynchronizedInvocationHandler handler = new SynchronizedInvocationHandler(realSubject);
    T result = makeProxy(subjectIntf, handler);
    handler.setLock(result);
    return result;
  }

  public static <T> T makeSynchronized(Class<T> subjectIntf, T realSubject, Object lock) {
    SynchronizedInvocationHandler handler = new SynchronizedInvocationHandler(realSubject);
    T result = makeProxy(subjectIntf, handler);
  ...
  private static <T> T makeProxy(Class<T> subjectIntf,SynchronizedInvocationHandler handler) {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      synchronized (lock) {
        ...
        if (method.getReturnType().isInterface()) {
          return makeSynchronized( (Class) method.getReturnType(), result, lock);
        }
        return result;
      }
    }
